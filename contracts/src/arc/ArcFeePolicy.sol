...
/// @notice Bounded policy for Arc, where the native EVM currency is USDC.
/// @dev Arc native USDC uses 18-decimal EVM value units. `2 ether` therefore
///      represents 2 USDC, not 2 ETH, on Arc.
contract ArcFeePolicy is TwoStepAdmin, IFeePolicy {
    ...
    uint256 public constant MAX_LAUNCH_FEE = 25 ether;
    ...
    function setTradeStakingShare(uint16 stakingShareBps) external onlyAdmin {
        ...
        uint16 previous = tradeStakingShareBps;
        tradeStakingShareBps = stakingShareBps;
        emit TradeStakingShareUpdated(previous, stakingShareBps);
    }
