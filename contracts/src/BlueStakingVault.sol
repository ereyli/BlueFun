...
/// @notice Non-custodial BLUE lock with streamed WETH revenue rewards and a cooldown exit.
/// @dev Administration can change future configuration but cannot seize active stakes or accounted rewards.
contract BlueStakingVault is TwoStepAdmin, ReentrancyGuard {
    ...
    constructor(
        ...
        if (
            stakingToken_ == address(0) || rewardToken_ == address(0) || guardian_ == address(0)
                || distributor_ == address(0) || stakingToken_ == rewardToken_ || stakingToken_.code.length == 0
    ...
    function stake(uint256 amount) external nonReentrant {
        ...
        uint256 beforeBalance = IERC20Balance(stakingToken).balanceOf(address(this));
        stakingToken.safeTransferFrom(msg.sender, address(this), amount);
        if (IERC20Balance(stakingToken).balanceOf(address(this)) - beforeBalance != amount) {
    ...
    function requestUnstake(uint256 amount) external nonReentrant {
        if (amount == 0) revert InvalidAmount();
        if (coolingBalanceOf[msg.sender] != 0) revert CooldownAlreadyActive();
    ...
    function cancelUnstake() external nonReentrant {
        if (emergencyExitEnabled) revert EmergencyMode();
        uint256 amount = coolingBalanceOf[msg.sender];
        if (amount == 0) revert NoCooldown();
        _updateReward(msg.sender);
    ...
    function withdraw(address recipient) external nonReentrant returns (uint256 amount) {
        ...
        amount = coolingBalanceOf[msg.sender];
        if (amount == 0) revert NoCooldown();
    ...
    function claimReward(address recipient) external nonReentrant returns (uint256 amount) {
        if (recipient == address(0)) revert InvalidAddress();
        _updateReward(msg.sender);
    ...
    function fundRewards(uint256 amount) external nonReentrant {
        ...
        uint256 beforeBalance = IERC20Balance(rewardToken).balanceOf(address(this));
        rewardToken.safeTransferFrom(msg.sender, address(this), amount);
        if (IERC20Balance(rewardToken).balanceOf(address(this)) - beforeBalance != amount) {
            revert UnexpectedTokenBehavior();
        }
        accountedRewardBalance += amount;
        _addRewards(amount);
    ...
    /// @notice Converts reward tokens sent directly to the vault into accounted rewards.
    function syncRewardSurplus() external nonReentrant returns (uint256 amount) {
        if (msg.sender != distributor) revert NotDistributor();
        if (rewardsArePaused) revert RewardsPaused();
        if (emergencyExitEnabled) revert EmergencyMode();
        uint256 balance = IERC20Balance(rewardToken).balanceOf(address(this));
        if (balance <= accountedRewardBalance) revert InsufficientSurplus();
        amount = balance - accountedRewardBalance;
    ...
    /// @notice Irreversible escape hatch. It unlocks users but never transfers their funds to administration.
    ...
    function recoverSurplusStakingToken(address recipient, uint256 amount) external onlyAdmin nonReentrant {
        ...
        uint256 balance = IERC20Balance(stakingToken).balanceOf(address(this));
    ...
    function recoverToken(address token, address recipient, uint256 amount) external onlyAdmin nonReentrant {
        if (token == stakingToken || token == rewardToken) revert ProtectedAsset();
        if (token == address(0) || recipient == address(0) || amount == 0) revert InvalidAmount();
        token.safeTransfer(recipient, amount);
        emit TokenRecovered(token, recipient, amount);
    }

    function recoverNative(address payable recipient, uint256 amount) external onlyAdmin nonReentrant {
