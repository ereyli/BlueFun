...
export async function POST(request: Request) {
  ...
  if ((chainId !== 8453 && chainId !== 4663 && chainId !== 143 && chainId !== 988 && chainId !== 5042) || !/^\d{1,32}$/.test(launchId) || !/^0x[a-fA-F0-9]{40}$/.test(token) || !/^0x[a-fA-F0-9]{40}$/.test(wallet) || !text || !Number.isFinite(timestamp) || Math.abs(Date.now() - timestamp) > 120_000 || !body.signature) {
    return NextResponse.json({ error: "Invalid signed message." }, { status: 400 });
  }
  const launch = chainId === 143 || chainId === 988 || chainId === 5042
