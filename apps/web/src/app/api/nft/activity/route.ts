...
export async function GET(request: Request) {
  ...
  return cachedResponse(`nft-activity:${collection}`, 10_000, () => loadActivity(collection));
}

async function loadActivity(collection: string) {
  ...
  const [mints, listings, transfers, ownerCount] = await Promise.all([
    ...
    db.from("nft_listings").select("marketplace,listing_id,seller,token_id,original_quantity,remaining_quantity,unit_price,end_time,cancelled,created_tx,updated_at").eq("chain_id", 8453).eq("collection", collection).order("updated_at", { ascending: false }).limit(1000),
  ...
  const listingIds = [...new Set(listingRows.map((row) => String(row.listing_id)))];
  const marketplaces = [...new Set(listingRows.map((row) => String(row.marketplace).toLowerCase()))];
  const sales = listingIds.length ? await db.from("nft_sales").select("marketplace,listing_id,buyer,recipient,quantity,gross_amount,tx_hash,log_index,created_at").eq("chain_id", 8453).in("marketplace", marketplaces).in("listing_id", listingIds).order("created_at", { ascending: false }).limit(100) : { data: [], error: null };
  const listingKey = (marketplace: unknown, listingId: unknown) => `${String(marketplace).toLowerCase()}:${String(listingId)}`;
  const listingById = new Map(listingRows.map((row) => [listingKey(row.marketplace, row.listing_id), row]));
  const saleRows = (sales.data || []).filter((row) => listingById.has(listingKey(row.marketplace, row.listing_id))).slice(0, 50);
  const activity = [
    ...
    ...listingRows.slice(0, 50).map((row) => ({ id: `listing-${row.marketplace}-${row.listing_id}`, type: "listing", tokenId: String(row.token_id), quantity: String(row.original_quantity), amount: String(BigInt(String(row.unit_price)) * BigInt(String(row.original_quantity))), wallet: row.seller, txHash: row.created_tx, createdAt: row.updated_at })),
    ...saleRows.map((row) => { const listing = listingById.get(listingKey(row.marketplace, row.listing_id)); return ({ id: `sale-${row.tx_hash}-${row.log_index}`, type: "sale", tokenId: String(listing?.token_id || "0"), quantity: String(row.quantity), amount: String(row.gross_amount), wallet: listing?.seller, counterparty: row.recipient || row.buyer, txHash: row.tx_hash, createdAt: row.created_at }); }),
  ...
  const totalVolume = saleRows.reduce((sum, row) => sum + BigInt(String(row.gross_amount)), 0n);
  const primaryMinted = (mints.data || []).reduce((sum, row) => sum + Number(row.quantity || 0), 0);
  ...
  return NextResponse.json({ activity, summary: { floorPrice: floorPrice?.toString() || null, totalVolume: totalVolume.toString(), sales: saleRows.length, mints: primaryMinted, listed: activeListings.length, owners } }, { headers: { "cache-control": "public, max-age=10, s-maxage=15, stale-while-revalidate=30" } });
}
