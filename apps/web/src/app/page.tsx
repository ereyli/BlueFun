...
export default async function HomePage({ searchParams }: { searchParams: Promise<{ chain?: string }> }) {
  ...
  const [baseOverview, robinhoodOverview, monadOverview, stableOverview] = await Promise.all([
    getCachedLaunchOverview(8453),
    getCachedLaunchOverview(4663),
    getCachedLaunchOverview(143),
    getCachedLaunchOverview(988)
  ]);
  const overview = chainId === 988 ? stableOverview : chainId === 143 ? monadOverview : chainId === 4663 ? robinhoodOverview : baseOverview;
  const { page, metrics } = overview;
  const fallback = page ? undefined : await (chainId === 143 || chainId === 988
  ...
  return <LaunchExplorer launches={launches} totalLaunches={page?.total ?? fallback?.length ?? launches.length} metrics={metrics} networkMetrics={{ 8453: baseOverview.metrics, 4663: robinhoodOverview.metrics, 143: monadOverview.metrics, 988: stableOverview.metrics }} chainId={chainId} />;
}
