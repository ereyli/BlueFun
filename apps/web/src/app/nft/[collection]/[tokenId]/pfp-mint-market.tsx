...
export function PFPMintMarket({ collection, tokenId, view = "item", deployment = "current" }: { collection: `0x${string}`; tokenId: bigint; view?: "item" | "collection"; deployment?: NFTDeployment }) {
  ...
  useEffect(() => { if (!uri.data) return; fetch(ipfsGateway(uri.data)).then((response) => response.ok ? response.json() : {}).then(setMetadata).catch(() => undefined); }, [uri.data]);
  useEffect(() => { if (!contractURI.data) return; fetch(ipfsGateway(contractURI.data)).then((response) => response.ok ? response.json() : {}).then(setCollectionMetadata).catch(() => undefined); }, [contractURI.data]);
  ...
  if (view === "collection") return <div className={`nft-home nft-drop-home ${mintFinished ? "mint-complete" : ""}`}>
    <NFTCollectionProfile collection={collection} name={name.data || "PFP collection"} symbol={symbol.data || "PFP"} description={collectionMetadata.description || metadata.description || (mintFinished ? "Primary mint is complete. Explore and trade the collection." : "Randomly assigned artwork with unique token IDs.")} image={collectionMetadata.image ? ipfsGateway(collectionMetadata.image) : metadata.image ? ipfsGateway(metadata.image) : undefined} creator={owner.data} standard="ERC-721" supply={maxSupply} minted={mintedCount} royaltyBps={BigInt(royaltyBps.data ?? 0)} status={mintFinished ? "Mint complete" : revealed.data ? "Live · Revealed" : "Live · Unrevealed"} socials={socialsFromMetadata(collectionMetadata)} mintPanel={!mintFinished ? primaryMintPanel : <div className="nft-compact-complete"><Sparkles/><strong>Mint complete</strong><span>Trading is live below.</span></div>}/>
  ...
  return <div className="pfp-detail-page">
    <div className="nft-item-page pfp-item-page"><section className="card nft-item-art">{metadata.image ? <img src={ipfsGateway(metadata.image)} alt={metadata.name || "PFP"}/> : <div><Sparkles/></div>}</section>
