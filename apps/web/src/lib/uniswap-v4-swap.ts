...
export function blueFunV4TokenPairPoolKey(
  token: `0x${string}`,
  quoteToken: `0x${string}`,
  config: { fee?: number; tickSpacing?: number; hooks?: `0x${string}` } = {}
): BlueFunV4PoolKey {
  const tokenFirst = BigInt(token) < BigInt(quoteToken);
  return {
    currency0: tokenFirst ? token : quoteToken,
    currency1: tokenFirst ? quoteToken : token,
    fee: config.fee ?? BLUEFUN_V4_POOL_FEE,
    tickSpacing: config.tickSpacing ?? BLUEFUN_V4_TICK_SPACING,
    hooks: config.hooks ?? zeroAddress
  };
...
export function buildV4TokenToTokenSwap({
  amountIn, amountOutMinimum, inputToken, outputToken, poolFee, tickSpacing, hooks, permit
}: {
  amountIn: bigint; amountOutMinimum: bigint; inputToken: `0x${string}`; outputToken: `0x${string}`;
  poolFee?: number; tickSpacing?: number; hooks?: `0x${string}`; permit?: Permit2Single;
}) {
  const poolKey = blueFunV4TokenPairPoolKey(inputToken, outputToken, { fee: poolFee, tickSpacing, hooks });
  const swapExactInSingle = encodeAbiParameters([{
    name: "swap", type: "tuple", components: [
  ...
  }], [{ poolKey, zeroForOne: inputToken.toLowerCase() === poolKey.currency0.toLowerCase(), amountIn, amountOutMinimum, hookData: "0x" }]);
  const settleAll = encodeAbiParameters([{ name: "currency", type: "address" }, { name: "maxAmount", type: "uint256" }], [inputToken, amountIn]);
  const takeAll = encodeAbiParameters([{ name: "currency", type: "address" }, { name: "minAmount", type: "uint256" }], [outputToken, amountOutMinimum]);
  const swapInput = encodeAbiParameters([{ name: "actions", type: "bytes" }, { name: "params", type: "bytes[]" }], ["0x060c0f", [swapExactInSingle, settleAll, takeAll]]);
  if (!permit) return { commands: "0x10" as const, inputs: [swapInput] };
  ...
  return { commands: "0x0a10" as const, inputs: [permitInput, swapInput] };
}
